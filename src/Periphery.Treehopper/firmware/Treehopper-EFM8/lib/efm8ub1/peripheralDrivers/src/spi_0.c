...
// Adaptive masked-window sizing for the USB/SPI lock-up fix.
// SPI0_pollTransfer services the FIFO in chunks with interrupts masked; the masked
// time per chunk is ~ chunk*(SPI0CKR+1) core clocks. We size the chunk from the
// live clock divider so that time stays roughly constant (~<=256us worst case)
// across the whole SPI range (~12 MHz down to ~94 kHz). This keeps USB un-starved
// even at slow SPI -- which is a real need (long/breadboard wiring, slow or
// isolated slaves, bring-up), not just a corner case.
//   chunk = clamp(SPI_MASK_BUDGET / (SPI0CKR + 1), 1, SPI_MASK_CHUNK_MAX)
#define SPI_MASK_CHUNK_MAX 32u   // cap at fast clocks (masked time is small there)
#define SPI_MASK_BUDGET   768u   // chunk*(SPI0CKR+1) budget (~256us at ~48 MHz sysclk)
                  ...
                  uint8_t xferCount)
{
  ...
  // USB/SPI lock-up fix, BOUNDED form. The polled FIFO loop races
  // with the USB ISR: a USB interrupt firing mid-transfer lets the in-flight TX
  // FIFO keep clocking into the 4-deep RX FIFO; on overflow RX bytes are dropped,
  // the xferCount accounting desyncs, and the loop spins forever. We service the
  // transfer in chunks with interrupts masked, capping TX at the chunk size so the
  // SPI is idle at each chunk boundary (nothing in flight) -- so reopening
  // interrupts between chunks can't reopen the overflow, and no drain is needed.
  // The chunk is sized from the live clock divider so the masked window stays
  // bounded across the whole SPI range (see SPI_MASK_BUDGET). Master-mode
  // transfers (all Treehopper SPI) always terminate.
  // Tiered (no 16-bit divide -- that pulled in ~140 B of C51 runtime and blew the
  // flash ceiling): halve the chunk each time the divider (SPI0CKR+1) doubles, so
  // chunk*(SPI0CKR+1) stays <= SPI_MASK_BUDGET (768 -> ~256us). Thresholds are
  // SPI0CKR <= 768/chunk - 1.
  {
    uint8_t ckr = SPI0CKR;                  // SPI clock divider - 1 (SYSCLK/(2*(ckr+1)))
    if      (ckr <= 23u)  maskChunk = 32u;  // >=1 MHz  : full chunk
    else if (ckr <= 47u)  maskChunk = 16u;
    else if (ckr <= 95u)  maskChunk = 8u;
    else if (ckr <= 191u) maskChunk = 4u;
    else                  maskChunk = 2u;   // down to ~94 kHz : 2-byte windows
  }

  while (xferCount)
  {
    uint8_t chunkRx = (xferCount < maskChunk) ? (uint8_t)xferCount : maskChunk;
    uint8_t chunkTx = chunkRx;   // send exactly as many as we read this chunk

    IE_EA = 0;   // mask: this chunk must be serviced un-preempted

    while (chunkRx)
    {
      // Feed TX, but ONLY up to this chunk's quota. Capping TX at chunkRx means
      // no bytes are left in flight when chunkRx hits 0, so the SPI is idle at
      // the unmask below -- without a drain loop (a drain that clocked out
      // in-flight TX without reading RX would overflow the RX FIFO and lose a
      // byte, which is what hung the first cut of this fix).
      if (chunkTx && SPI0CN0_TXNF && txCount)
      {
        if (dir & SPI0_TRANSFER_TX) { SPI0DAT = *pTxBuffer; ++pTxBuffer; }
        else                        { SPI0DAT = 0; }
        --txCount; --chunkTx;
      }

      // Read RX as bytes arrive
      if (!(SPI0CFG & SPI0CFG_RXE__BMASK))
      {
        if (dir & SPI0_TRANSFER_RX) { *pRxBuffer = SPI0DAT; ++pRxBuffer; }
        else                        { uint8_t dummy = SPI0DAT; }
        --xferCount; --chunkRx;
      }

      // Slave 4-wire: bail if the master deselected (no-op in master mode).
      if (!modeIsMaster && useNss)
      {
        if (!checkNss) { if (SPI0CFG & SPI0CFG_SLVSEL__BMASK) checkNss = true; }
        else if (!(SPI0CFG & SPI0CFG_SLVSEL__BMASK)) { xferCount = 0; break; }
      }
    }

    // chunkTx == chunkRx == 0: exactly `chunk` bytes sent and received, so the
    // SPI is idle (nothing in flight) and reopening interrupts here is safe.
    IE_EA = 1;
  }
