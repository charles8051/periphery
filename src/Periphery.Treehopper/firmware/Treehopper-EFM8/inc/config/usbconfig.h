...
// -----------------------------------------------------------------------------
// $[Power Save Mode]
// USB_PWRSAVE_MODE_OFF, not ONSUSPEND, because the watchdog is enabled.
//
// Under ONSUSPEND the device parks in USBD_Suspend()'s loop (efm8_usbd.c) and calls
// USB_SuspendOscillator(), which sets PCON1.SUSPEND and halts the core and peripheral
// clocks. The watchdog does NOT stop with them: EFM8UB1 RM sec. 23.1 states "when the WDT
// is active, the low-frequency oscillator is forced on", so it keeps counting and would
// reset the board on any suspend outlasting the interval. The main-loop feed cannot help,
// because the core is parked inside the USB ISR with the superloop not running.
//
// Feeding through suspend is not a workable alternative: only a Timer 4 event wakes the
// core (RM Table 7.1 - Timer 3 is not a wake source), and waking every tick to feed defeats
// the point of suspending at all.
//
// Cost is USB suspend-current compliance, which is acceptable here: these boards are
// permanently attached inside a kiosk, the device already reports itself self-powered
// (USBD_IsSelfPoweredCb, callback.c), and Silicon Labs' own USB factory bootloader omits
// suspend/resume entirely while still passing chapter 9 (AN945 sec. 5.3.2).
//
// Side benefit: this removes the USB-suspend hard-wedge escalation path documented in
// docs/investigations/2026-06-treehopper-spi-usb-lockup.md.
