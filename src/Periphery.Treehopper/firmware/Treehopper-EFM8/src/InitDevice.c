...
WDT_0_enter_DefaultMode_from_RESET (void)
{
  ...
  // The WDT is left ENABLED (Treehopper.hwconf: wdt.watchdogcontrol.wdtenable = Enable).
  //
  // Nothing to do here. Per the EFM8UB1 reference manual sec. 23.3, "The WDT is enabled
  // and reset as a result of any system reset" - WDTCN resets to 0x17 (bit 4 set = active,
  // bits[2:0] = 111b = the maximum ~13.1 s interval at 80 kHz LFOSC). Enabling is therefore
  // a no-op; the watchdog is already running by the time this executes.
  //
  // The erratum WDT_E102 feed-then-delay dance that used to live here existed solely to make
  // the *disable* safe. With no disable, it is dead weight (and its uint32_t loop counter
  // pulled in the ?C?ULCMP library helper for a 32-bit compare).
  //
  // The feed lives in the main superloop (main.c) and the disable-lockout in Treehopper_Init()
  // - see the rationale in both places.
  // [WDTCN_2 - Watchdog Timer Control]$
