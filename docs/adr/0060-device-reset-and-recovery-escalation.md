...
**`Attempt` follows the same rule.** The first implementation of this decision preserved only `ResetCount`. `RecoveryContext.Attempt` lived in a loop local that restarted at 1 whenever a reopen succeeded, so a policy that indexes its steps by `Attempt` (`EscalatingResetRecoveryPolicy`) still restarted at its first step every cycle. The attempt count is now proxy state, cleared at the same two points as `ResetCount`.
