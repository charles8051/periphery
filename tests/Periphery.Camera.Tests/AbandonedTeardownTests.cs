...
public sealed class AbandonedTeardownTests : IDisposable
{
    ...
    // ── The refusal expires (ADR-0084 D5) ──────────────────────────────

    // Before this, an abandoned step that never returned refused its device for
    // the life of the process — and the exception's own advice, replug the
    // camera, could not lift it, because the registry never sees PnP edges. The
    // refusal is now bounded: past the window the open is attempted again.
    [Fact]
    public async Task RefusalLapsesOnceTheWindowElapses_EvenThoughTheStepIsStillRunning()
    {
        var time = new FakeTimeProvider();
        var stopGate = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var device = TestHelpers.CreateDeviceInfo(@"TEST\CAM\LAPSES");
        var backend = new InMemoryCameraBackend { BlockStopUntil = stopGate.Task };
        ...
        try
        {
            ...
            await Assert.ThrowsAsync<CameraTeardownPendingException>(() => CameraDevice.OpenAsync(device));

            time.Advance(PendingTeardowns.RefusalTtl);

            // The stop is still parked — nothing about the device got better —
            // but the registry has stopped standing in the way.
            Assert.False(stopGate.Task.IsCompleted);
            Assert.Null(PendingTeardowns.Find(device.Id));
    ...
    // "Await Completion and retry" has to terminate. A step wedged in a driver
    // call never completes, so the signal is bounded by the same window that
    // bounds the refusal.
    [Fact]
    public async Task ExceptionCompletion_IsBoundedByTheRefusalWindow_NotByTheWedgedStep()
    {
        var time = new FakeTimeProvider();
        var stopGate = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var device = TestHelpers.CreateDeviceInfo(@"TEST\CAM\BOUNDED");
        var backend = new InMemoryCameraBackend { BlockStopUntil = stopGate.Task };
    ...
    // A second abandonment after the window has lapsed opens a fresh window.
    // Composing onto the lapsed entry would carry its first-abandoned timestamp
    // forward, so the new step would be born expired and refuse nothing.
    [Fact]
    public void AbandonmentAfterALapsedWindowStartsAFreshOne()
    {
        var clock = new FakeTimeProvider();
        var id = @"TEST\CAM\RELAPSE";
        ...
        try
        {
            ...
            clock.Advance(PendingTeardowns.RefusalTtl);
            Assert.Null(PendingTeardowns.Find(id));

            PendingTeardowns.Register(id, BoundedTeardown.Steps.BackendDispose, second.Task, clock);

            var entry = PendingTeardowns.Find(id);
            Assert.NotNull(entry);
            Assert.True(entry.PendingFor < PendingTeardowns.RefusalTtl);
            Assert.Equal([BoundedTeardown.Steps.BackendDispose], entry.Steps);
        }
        finally
        {
            first.TrySetResult();
